...
contract('Owner Manager', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    ownerManager = await OwnerManager.new(token.address, {
      from: tokeny
    });
    ...
    await user1Contract.addKey(actionKey, 2, 1, {
      from: user1
    }).should.be.fulfilled;
    ...
    await ownerManager.addOwnerAdmin(tokeny, {
      from: tokeny
    });
    await identityRegistryStorage.transferOwnershipOnIdentityRegistryStorage(ownerManager.address, {
      from: tokeny
    });
    await token.transferOwnershipOnTokenContract(ownerManager.address, {
      from: tokeny
    });
    await identityRegistry.transferOwnershipOnIdentityRegistryContract(ownerManager.address, {
      from: tokeny
    });
    await claimTopicsRegistry.transferOwnershipOnClaimTopicsRegistryContract(ownerManager.address, {
      from: tokeny
    });
    await trustedIssuersRegistry.transferOwnershipOnIssuersRegistryContract(ownerManager.address, {
      from: tokeny
    });
    await defaultCompliance.transferOwnershipOnComplianceContract(ownerManager.address, {
      from: tokeny
    });
  });
  ...
  it('Should add and remove ownerAdmin role on OwnerManager', async () => {
    ...
    await ownerManager.addOwnerAdmin(admin, {
      from: user1
    ...
    await ownerManager.removeOwnerAdmin(admin, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isOwnerAdmin(admin)).should.be.equal(true);
  ...
  it('Should add and remove registryAddressSetter role on OwnerManager', async () => {
    ...
    // try to add role without being admin
    await ownerManager.addRegistryAddressSetter(registrySetter, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    ...
    // try to remove role without being admin
    await ownerManager.removeRegistryAddressSetter(registrySetter, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isRegistryAddressSetter(registrySetter)).should.be.equal(true);
    // remove role as admin
  ...
  it('Should add and remove complianceSetter role on OwnerManager', async () => {
    ...
    // try to add role without being admin
    await ownerManager.addComplianceSetter(complianceSetter, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    ...
    // try to remove role without being admin
    await ownerManager.removeComplianceSetter(complianceSetter, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isComplianceSetter(complianceSetter)).should.be.equal(true);
    // remove role as admin
  ...
  it('Should add and remove claimRegistryManager role on OwnerManager', async () => {
    ...
    // try to add role without being admin
    await ownerManager.addClaimRegistryManager(claimRegistryManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    ...
    // try to remove role without being admin
    await ownerManager.removeClaimRegistryManager(claimRegistryManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isClaimRegistryManager(claimRegistryManager)).should.be.equal(true);
    // remove role as admin
  ...
  it('Should add and remove issuersRegistryManager role on OwnerManager', async () => {
    ...
    // try to add role without being admin
    await ownerManager.addIssuersRegistryManager(issuersRegistryManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    ...
    // try to remove role without being admin
    await ownerManager.removeIssuersRegistryManager(issuersRegistryManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isIssuersRegistryManager(issuersRegistryManager)).should.be.equal(true);
    // remove role as admin
  ...
  it('Should add and remove tokenInfoManager role on OwnerManager', async () => {
    ...
    // try to add role without being admin
    await ownerManager.addTokenInfoManager(tokenInfoManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    ...
    // try to remove role without being admin
    await ownerManager.removeTokenInfoManager(tokenInfoManager, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    (await ownerManager.isTokenInfoManager(tokenInfoManager)).should.be.equal(true);
    // remove role as admin
  ...
  it('Should set identity registry only if onchainID is registered as registryAddressSetter', async () => {
    ...
    await ownerManager.addRegistryAddressSetter(registrySetterID.address, {
      from: tokeny
    });
    ...
    await ownerManager.callSetIdentityRegistry(identityRegistry2.address, registrySetterID.address, {
      from: registrySetter
    });
    ...
    await ownerManager
      .callSetIdentityRegistry(identityRegistry2.address, registrySetterID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
    (await token.identityRegistry()).should.be.equal(identityRegistry2.address);
  ...
  it('Should set topics registry only if onchainID is registered as registryAddressSetter', async () => {
    ...
    const claimTopicsRegistry2 = await ClaimTopicsRegistry.new({
      from: registrySetter
    });
    // set claim topics registry on the identity registry contract
    await ownerManager.callSetClaimTopicsRegistry(claimTopicsRegistry2.address, registrySetterID.address, {
      from: registrySetter
    });
    (await identityRegistry.topicsRegistry()).should.be.equal(claimTopicsRegistry2.address);
    // should not work if wallet is not set as management key on the onchainID
    await ownerManager
      .callSetClaimTopicsRegistry(claimTopicsRegistry2.address, registrySetterID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
    (await identityRegistry.topicsRegistry()).should.be.equal(claimTopicsRegistry2.address);
  ...
  it('Should set trusted issuers registry only if onchainID is registered as registryAddressSetter', async () => {
    ...
    const trustedIssuersRegistry2 = await TrustedIssuersRegistry.new({
      from: registrySetter
    });
    // set trusted issuers registry on the identity registry contract
    await ownerManager.callSetTrustedIssuersRegistry(trustedIssuersRegistry2.address, registrySetterID.address, {
      from: registrySetter
    });
    (await identityRegistry.issuersRegistry()).should.be.equal(trustedIssuersRegistry2.address);
    // should not work if wallet is not set as management key on the onchainID
    await ownerManager
      .callSetTrustedIssuersRegistry(trustedIssuersRegistry2.address, registrySetterID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
    (await identityRegistry.issuersRegistry()).should.be.equal(trustedIssuersRegistry2.address);
  ...
  it('Should set compliance contract only if onchainID is registered as registryAddressSetter', async () => {
    ...
    await ownerManager.addComplianceSetter(complianceSetterID.address, {
      from: tokeny
    });
    (await ownerManager.isComplianceSetter(complianceSetterID.address)).should.be.equal(true);
    // create new compliance contract
    const compliance2 = await Compliance.new({
      from: complianceSetter
    });
    // set compliance on the token contract
    await ownerManager.callSetCompliance(compliance2.address, complianceSetterID.address, {
      from: complianceSetter
    });
  ...
  it('Should add trusted issuer to the registry only if onchainID is registered as IssuersRegistryManager', async () => {
    ...
    const claimIssuerContract2 = await IssuerIdentity.new(claimIssuer2, {
      from: claimIssuer2
    });
    // add new issuersRegistryManager
    await ownerManager.addIssuersRegistryManager(issuersRegistryManagerID.address, {
      from: tokeny
    });
    (await ownerManager.isIssuersRegistryManager(issuersRegistryManagerID.address)).should.be.equal(true);
    // should not work if wallet is not set as management key on the onchainID
    await ownerManager
      .callAddTrustedIssuer(claimIssuerContract2.address, claimTopics, issuersRegistryManagerID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
  ...
  it('Should remove trusted issuer from the registry only if onchainID is registered as IssuersRegistryManager', async () => {
    ...
    await ownerManager
      .callRemoveTrustedIssuer(claimIssuerContract2.address, issuersRegistryManagerID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
    (await trustedIssuersRegistry.isTrustedIssuer(claimIssuerContract2.address)).should.be.equal(true);
    // remove should work if wallet is set as management key on the onchainID
    await ownerManager.callRemoveTrustedIssuer(claimIssuerContract2.address, issuersRegistryManagerID.address, {
      from: issuersRegistryManager
    });
    (await trustedIssuersRegistry.isTrustedIssuer(claimIssuerContract2.address)).should.be.equal(false);
  });
  ...
  it('Should update trusted issuer claim topics only if onchainID is registered as IssuersRegistryManager', async () => {
    ...
    await ownerManager
      .callUpdateIssuerClaimTopics(claimIssuerContract.address, claimTopics2, issuersRegistryManagerID.address, {
        from: tokeny
      })
      .should.be.rejectedWith(EVMRevert);
  ...
  it('Should set token name only if onchainID is registered as TokenInfoManager', async () => {
    ...
    await ownerManager.callSetTokenName('TREXDINO1', tokenInfoManagerIdentity.address, {
      from: tokenInfoManager
    }).should.be.rejectedWith(EVMRevert);

    // should set information if sender is token info manager
    await ownerManager.addTokenInfoManager(tokenInfoManagerIdentity.address, {
      from: tokeny
    }).should.be.fulfilled;
  ...
  it('Should set token onchain ID only if sender onchainID is registered as TokenInfoManager', async () => {
    ...
    // should revert if sender is not token info manager
    await ownerManager
      .callSetTokenOnchainID('0x0000000000000000000000000000000000000000', tokenInfoManagerIdentity.address, {
        from: tokenInfoManager
      })
      .should.be.rejectedWith(EVMRevert);
  ...
  it('Should set token symbol only if onchainID is registered as TokenInfoManager', async () => {
    ...
    const tokenInfoManagerIdentity = await deployIdentityProxy(tokenInfoManager);
    // should revert if sender is not token info manager
    await ownerManager.callSetTokenSymbol('TREX1', tokenInfoManagerIdentity.address, {
      from: tokenInfoManager
    }).should.be.rejectedWith(EVMRevert);
  ...
  it('Should add claim topic in the claim topics registry only if onchainID is registered as ClaimRegistryManager', async () => {
    ...
    await ownerManager.callAddClaimTopic(5, claimRegistryManagerIdentity.address, {
      from: claimRegistryManager
    }).should.be.rejectedWith(EVMRevert);

    // should add claim topic if sender is claim registry manager
    await ownerManager.addClaimRegistryManager(claimRegistryManagerIdentity.address, {
      from: tokeny
    });
  ...
  it('Should remove claim topic in the claim topics registry only if onchainID is registered as ClaimRegistryManager', async () => {
    ...
    // should revert if sender is not claim registry manager
    await ownerManager
      .callRemoveClaimTopic(7, claimRegistryManagerIdentity.address, {
        from: claimRegistryManager
      })
      .should.be.rejectedWith(EVMRevert);
    ...
    (await ownerManager.isClaimRegistryManager(claimRegistryManagerIdentity.address)).should.be.equal(true);
    await ownerManager.callRemoveClaimTopic(7, claimRegistryManagerIdentity.address, {
      from: claimRegistryManager
    }).should.be.fulfilled;
  ...
  it('Should transfer ownership of contracts if called by admin', async () => {
    ...
    await ownerManager.callTransferOwnershipOnTokenContract(newOwner, {
      from: tokeny
    }).should.be.fulfilled;
    (await token.owner()).should.equal(newOwner);
    await ownerManager.callTransferOwnershipOnIdentityRegistryContract(newOwner, {
      from: tokeny
    }).should.be.fulfilled;
    (await identityRegistry.owner()).should.equal(newOwner);
    await ownerManager.callTransferOwnershipOnComplianceContract(newOwner, {
      from: tokeny
    }).should.be.fulfilled;
    (await defaultCompliance.owner()).should.equal(newOwner);
    await ownerManager.callTransferOwnershipOnClaimTopicsRegistryContract(newOwner, {
      from: tokeny
    }).should.be.fulfilled;
    (await claimTopicsRegistry.owner()).should.equal(newOwner);
    await ownerManager.callTransferOwnershipOnIssuersRegistryContract(newOwner, {
      from: tokeny
    }).should.be.fulfilled;
  ...
  it('Should add and remove agent in token contract', async () => {
    ...
    await ownerManager.callAddAgentOnTokenContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
    (await token.isAgent(newAgent)).should.equal(true);
    await ownerManager.callRemoveAgentOnTokenContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
  ...
  it('Should add and remove agent in identity registry contract', async () => {
    const newAgent = accounts[6];
    await ownerManager.callAddAgentOnIdentityRegistryContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
    (await identityRegistry.isAgent(newAgent)).should.equal(true);
    await ownerManager.callRemoveAgentOnIdentityRegistryContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
  ...
  it('Should transfer ownership of compliance if called by compliance manager with custom function call', async () => {
    ...
    await ownerManager.addComplianceManager(complianceManagerID.address, {
      from: tokeny
    });
    ...
    await ownerManager.callComplianceFunction(callData, complianceManagerID.address, {
