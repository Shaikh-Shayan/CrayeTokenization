...
contract('Token', (accounts) => {
  ...
  it('allowance returns the approved amount of tokens', async () => {
    ...
    const tx = await token.approve(user2, 500, {
      from: user1
    }).should.be.fulfilled;
...
contract('Token', (accounts) => {
  ...
  it('should decrease allowance by the amount of tokens given', async () => {
    await token.approve(user2, 500, {
      from: user1
    }).should.be.fulfilled;
    const tx = await token.decreaseAllowance(user2, 100, {
      from: user1
    });
  ...
  it('Successful Token transfer', async () => {
    ...
    await token.transfer('0x0000000000000000000000000000000000000000', 300, {
      from: user1
    }).should.be.rejectedWith(Error);
    const tx = await token.transfer(user2, 300, {
      from: user1
    }).should.be.fulfilled;
  ...
  it('Should proceed a batch of 20 transfers', async () => {
    ...
    const tx3 = await token.transfer(user2, 100, {
      from: user1
    });
  ...
  it('Token transfer fails if claim signer key is removed from trusted claim issuer contract', async () => {
    await claimIssuerContract.removeKey(signerKey, 3, {
      from: claimIssuer
    });
    await token.transfer(user2, 300, {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    const balance1 = await token.balanceOf(user1);
    const balance2 = await token.balanceOf(user2);
  ...
  it('Token transfer fails if a users identity is removed from identity registry', async () => {
    await identityRegistry.deleteIdentity(user2, {
      from: agent
    });
    await token.transfer(user2, 300, {
      from: user1
  ...
  it('Token transfer fails if claimTopic is removed from claimTopic registry', async () => {
    await claimTopicsRegistry.removeClaimTopic(7, {
      from: tokeny
    });
    await claimTopicsRegistry.addClaimTopic(8, {
      from: tokeny
    });
    await token.transfer(user2, 300, {
      from: user1
  ...
  it('Token transfer fails if claimId is revoked', async () => {
    ...
    // user2 adds claim to identity contract
    await user2Contract.addClaim(3, 1, claimIssuerContract.address, signature2, hexedData2, '', {
      from: user2
    }).should.be.fulfilled;
  ...
  it('Token transfer passes if same topic claim added by different issuer', async () => {
    ...
    // Claim issuer deploying identity contract
    const claimIssuer2Contract = await IssuerIdentity.new(claimIssuer2, {
      from: claimIssuer2
    });

    // Claim issuer adds claim signer key to his contract
    await claimIssuer2Contract.addKey(signerKey, 3, 1, {
      from: claimIssuer2
    }).should.be.fulfilled;
    ...
    // user2 adds claim to identity contract
    await user2Contract.addClaim(7, 1, claimIssuer2Contract.address, signature2, hexedData2, '', {
      from: user2
    }).should.be.fulfilled;
  ...
  it('Token transfer fails if trusted issuer do not have claim topic', async () => {
    ...
    // Tokeny adds trusted claim Issuer to claimIssuer registry
    await trustedIssuersRegistry.addTrustedIssuer(claimIssuer2Contract.address, [0], {
      from: tokeny
    }).should.be.fulfilled;
    ...
    // user2 adds claim to identity contract
    await user2Contract.addClaim(3, 1, claimIssuer2Contract.address, signature2, hexedData2, '', {
      from: user2
    }).should.be.fulfilled;
  ...
  it('Successful Burn the tokens', async () => {
    ...
    await token.burn('0x0000000000000000000000000000000000000000', 300, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
    const tx = await token.burn(user1, 300, {
      from: agent
    }).should.be.fulfilled;
  ...
  it('Should proceed a batch of 20 burn transactions', async () => {
    ...
    const balance1 = await token.balanceOf(user1);
    balance1.toString().should.equal('600');
    const tx3 = await token.burn(user1, 100, {
      from: agent
    });
  ...
  it('batchBurn should fail if not called by an agent', async () => {
    await token.batchMint([user1, user2], [1000, 1000], {
      from: agent
    }).should.be.fulfilled;
    await token.batchBurn([user1, user2], [1000, 1000], {
      from: user1
    }).should.be.rejectedWith(EVMRevert);
    const balance1 = await token.balanceOf(user1);
    const balance2 = await token.balanceOf(user2);
  ...
  it('Should remove agent from token contract', async () => {
    ...
    const tx1 = await token.addAgentOnTokenContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
    ...
    const tx2 = await token.removeAgentOnTokenContract(newAgent, {
      from: tokeny
    }).should.be.fulfilled;
    log(
  ...
  it('Wallet recovery should be successful and freeze status should be transferred', async () => {
    ...
    // identity contracts are registered in identity registry
    await identityRegistry.registerIdentity(accounts[7], user11Contract.address, 91, {
      from: agent
    }).should.be.fulfilled;
    ...
    await user11Contract.addClaim(7, 1, claimIssuerContract.address, signature11, hexedData11, '', {
      from: tokeny
    });

    // tokeny mint the tokens to the accounts[7]
    await token.mint(accounts[7], 1000, {
      from: agent
    });
    ...
    await user11Contract.addKey(key, 1, 1, {
      from: tokeny
    });
    await token.setAddressFrozen(accounts[7], true, {
      from: agent
    });
    await token.freezePartialTokens(accounts[7], 200, {
      from: agent
    });

    // tokeny recover the lost wallet of accounts[7]
    const tx = await token.recoveryAddress(accounts[7], accounts[8], user11Contract.address, {
      from: agent
    }).should.be.fulfilled;
  ...
  it('Recovery should fail if the new wallet does not have a management key on the onchainID', async () => {
    ...
    // tokeny recover the lost wallet of accounts[7]
    await token.recoveryAddress(accounts[7], accounts[8], user11Contract.address, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
    const balance1 = await token.balanceOf(accounts[7]);
    const balance2 = await token.balanceOf(accounts[8]);
  ...
  it('Should revert freezing if amount exceeds available balance', async () => {
    await token.freezePartialTokens(user1, 1100, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
  });

  it('Should revert unfreezing if amount exceeds available balance', async () => {
    await token.unfreezePartialTokens(user1, 500, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
  });

  it('Token transfer fails if amount exceeds unfrozen tokens', async () => {
    const tx = await token.freezePartialTokens(user1, 800, {
      from: agent
    });
  ...
  it('Should proceed a batch of 20 freezePartialTokens transactions', async () => {
    ...
    balance1.toString().should.equal('700');
    balance2.toString().should.equal('300');
    const tx3 = await token.freezePartialTokens(user1, 100, {
      from: agent
    });
  ...
  it('Tokens transfer after unfreezing tokens', async () => {
    await token.freezePartialTokens(user1, 800, {
      from: agent
    });
    const frozenTokens1 = await token.getFrozenTokens(user1);
    const tx = await token.unfreezePartialTokens(user1, 500, {
      from: agent
    });
  ...
  it('Should proceed a batch of 20 unfreezePartialTokens transactions', async () => {
    ...
    frozenTokens2.toString().should.equal('0');
    const tx3 = await token.unfreezePartialTokens(user1, 100, {
      from: agent
    });
    const frozenTokens3 = await token.getFrozenTokens(user1);
  ...
  it('Successfuly transfers Token if sender approved', async () => {
    ...
    await token.approve(accounts[4], 300, {
      from: user1
    }).should.be.fulfilled;
    const tx = await token.transferFrom(user1, user2, 300, {
      from: accounts[4]
    }).should.be.fulfilled;
  ...
  it('Transfer fails if identity registry not verified', async () => {
    ...
    await token.transferFrom(user1, accounts[4], 300, {
      from: accounts[4]
    }).should.be.rejectedWith(EVMRevert);
  });
  ...
  it('Transfer from fails if amount exceeds unfrozen tokens', async () => {
    ...
    await token.transferFrom(user1, user2, 300, {
      from: accounts[4]
    }).should.be.rejectedWith(EVMRevert);
    const balance2 = await token.balanceOf(user1);
    balance1.toString().should.equal('1000');
  ...
  it('Token transfer fails if address is frozen', async () => {
    const tx = await token.setAddressFrozen(user1, true, {
      from: agent
    });
  ...
  it('Token transfer from fails if address is frozen', async () => {
    await token.setAddressFrozen(user1, true, {
      from: agent
    });
    await token.approve(accounts[4], 300, {
      from: user1
  ...
  it('Successful forced transfer', async () => {
    const tx = await token.forcedTransfer(user1, user2, 300, {
      from: agent
    }).should.be.fulfilled;
  ...
  it('Should proceed a batch of 20 forcedTransfer transactions', async () => {
    ...
    balance1.toString().should.equal('600');
    balance2.toString().should.equal('400');
    const tx3 = await token.forcedTransfer(user1, user2, 100, {
      from: agent
    });
    const balance3 = await token.balanceOf(user1);
    const balance4 = await token.balanceOf(user2);
  ...
  it('Forced transfer successful between frozen addresses', async () => {
    ...
    await token.setAddressFrozen(user2, true, {
      from: agent
    });
    await token.forcedTransfer(user1, user2, 300, {
      from: agent
    }).should.be.fulfilled;
    const balance1 = await token.balanceOf(user1);
    const balance2 = await token.balanceOf(user2);
  ...
  it('Forced transfer fails if balance is not enough', async () => {
    await token.forcedTransfer(user1, user2, 1200, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
  });

  it('Forced transfer fails if identity is not verified', async () => {
    await token.forcedTransfer(user1, accounts[4], 300, {
      from: agent
    }).should.be.rejectedWith(EVMRevert);
  });
  ...
  it('Should proceed a batch of 20 mint transactions', async () => {
    ...
    const tx3 = await token.mint(user1, 1000, {
      from: agent
    }).should.be.fulfilled;
