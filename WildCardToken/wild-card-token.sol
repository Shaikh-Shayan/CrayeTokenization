...
contract WildCardTokenAirdrop is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(uint256 _initialSupply, address _propertyTokensOwner) ERC20("WildCard", "WCD") {
        _mint(msg.sender, _initialSupply);
        propertyTokensOwner = _propertyTokensOwner;
    }

    function claimPropertyTokens (
        uint256 _noOfPropertyTokens
        ) external returns (bool){
            //check if the user has the required number of Wild Card tokens or not
            require(
                balanceOf(msg.sender)>=_noOfPropertyTokens,
                "Wild Card Tokens Insufficient!"
            );
